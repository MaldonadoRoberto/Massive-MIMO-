...
for nn = 1:2
    ...
    for i = 1:length(n_BS)  % Se recorre el vector de M
        ...
        correlation_c(i) = mean(correlacion_rep);
        correlation_c_iid(i) = mean(correlacion_rep_iid);
    end
    
    % Valor del coeficiente de correlación para un numero de antenas M
    % dado.
    correlation(:,nn) = correlation_c;
end
...
correlation_iid_inter = interp1(n_BS,correlation_c_iid,n_bs_nuevo,'spline');
...
plot(n_BS,correlation_c_iid,'ok')
grid on
legend('LOS','NLOS','IID')
...
plot(n_BS,correlation_c_iid,'o--k','LineWidth',1.5)
grid on
legend('LOS','NLOS','IID')
xlabel('Número de antenas en transmisión')
ylabel('Coeficiente de correlación')

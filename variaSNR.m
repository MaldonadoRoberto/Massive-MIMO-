...
%%%%       para un sistema de MIMO masivo en función de la SNR media   %%%%
%%%%       del sistema. Se puede utilizar tanto para el enlace uplink  %%%%
%%%%              como para downlink. El número de antenas en BS       %%%%
%%%%            y el número de usuarios permanece constante            %%%%
...
for i = 1:iteraciones
    ...
    for k = 1:length(index)
        ...
        % Seleción del enlace que se quiere simular
        if DL == 1 
    ...
    % Obtención de la media sobre las k realizaciones
    CMRT(i,:) = mean(C_mrt);
    CZF(i,:) = mean(C_zf);
    CMMSE(i,:) = mean(C_mmse);
    CDPC(i,:) = mean(C_dpc);

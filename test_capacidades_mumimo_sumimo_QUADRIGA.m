...
[canalQ, canalIID,ch_par] = generaCanalQuadriga(5,M,K,1,1,0,'NLOS');
...
for t = 1 : K  
    ...
    SINR = ((alpha)*(P_transmitida)*norm(H(:,t).'*W(:,t))^2)/(sum_v*P_transmitida*alpha+P_ruido);  % Ecuación (2.28)
...
[SNR_media, SNR_mediaDB] = calculaSNRmedia(ch_par,P_transmitida,P_ruido);

C_SUMIMO = log2(det(eye(K)+(P_transmitida/P_ruido)*H*Kx*H'));
...
variable p(K);
maximize (-det_inv(eye(M)+ (P_transmitida/P_ruido)*conj(H)*diag(p)*H.'));
subject to
    sum(p) == Pmax
    min(p)>=0
cvx_end
C_MUMIMO = log2(det(eye(M)+(P_transmitida/P_ruido)*conj(H)*diag(p)*H.'));  %Formula (2.6) de la tesis

[SNRmedia,SNRmediadB] = calculaSNRmedia(ch_par,P_transmitida,P_ruido);

potencia_asignada = wf_norm(H,SNRmedia);
C_MUMIMO_WF = log2(det(eye(M)+(P_transmitida/P_ruido)*conj(H)*diag(potencia_asignada)*H.'));


% Si realizamos el mismo método pero sin optmización de potencias tenemos:
P_MUMIMO = eye(K)*1/K;
C_MUMIMO_equipotencia = log2(det(eye(M)+(P_transmitida/P_ruido)*conj(H)*P_MUMIMO*H.'));
